...
export default function PreferuaratPage() {
  ...
  useEffect(() => {
    async function fetchFavorites() {
      ...
      if (data) setMovies(data.map((f: any) => f.movies).filter(Boolean))
      setLoading(false)
    }
    fetchFavorites()
  }, [])
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#fff', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: '40px 60px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 600, marginBottom: '24px' }}>❤️ Të Preferuarat</h1>
        {loading && <p style={{ color: '#6b6b80' }}>Duke ngarkuar...</p>}
        {!loading && movies.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>❤️</div>
            <p style={{ color: '#6b6b80', marginBottom: '16px' }}>Nuk ke shtuar asnjë film te të preferuarat.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '12px' }}>
          {movies.map((m: any) => (
            <Link key={m.id} href={`/film/${m.slug}`} style={{ textDecoration: 'none' }}>
              <div style={{ borderRadius: '8px', overflow: 'hidden', background: '#12121a' }}>
